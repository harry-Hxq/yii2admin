...
/* 页面基本属性 */
$this->title = '停车管理';
$this->params['title_sub'] = '订单列表';  // 在\yii\base\View中有$params这个可以在视图模板中共享的参数
    ...
    [
        ...
        'content' => function($model){
            return $model['status'] == 4 ?
                Html::tag('span',Yii::$app->params['PAY_STATUS'][$model['status']],['class'=>'badge badge-success']) :
                Html::tag('span',Yii::$app->params['PAY_STATUS'][$model['status']],['class'=>'badge badge-important']);
        }
    ],
    [
        'header' => '创建时间',
...
jQuery(document).ready(function() {
    highlight_subnav('user-recharge/index'); //子导航高亮
});
<?php $this->endBlock() ?>
<!-- 将数据块 注入到视图中的某个位置 -->
